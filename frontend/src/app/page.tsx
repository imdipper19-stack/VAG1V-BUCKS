...
const FAQS = [
  ['Как быстро придут V-Bucks?', 'Обычно 5-15 минут после оплаты и авторизации. ногда обработка может занять дольше, если Epic Games временно ограничивает запросы.'],
...
const LIVE_ORDERS = [
  ['#VB-2418', '2 400 V-Bucks', '7 минут', 'Выдано'],
  ['#VB-2417', '800 V-Bucks',   '5 минут', 'Выдано'],
  ['#VB-2416', '4 500 V-Bucks', '11 минут', 'Выдано'],
  ['#VB-2415', '12 500 V-Bucks', '14 минут', 'Выдано'],
];
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    ordersApi
      .getQueueInfo()
      .then((res) => {
        if (res.success) setQueueSize(res.data.queueSize);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <main className="landing-page">
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description: 'Купить V-Bucks для Fortnite по выгодной цене. Автоматическая выдача за 5-15 минут.',
            potentialAction: {
              '@type': 'SearchAction',
      ...
      <div className="landing-rail" aria-hidden="true">
        <div className="landing-rail-dot active">01</div>
        <div className="landing-rail-dot">02</div>
        <div className="landing-rail-dot">03</div>
        <div className="landing-rail-dot">04</div>
      </div>

      <div className="landing-shell">
        <nav className="landing-nav landing-reveal">
          <a className="landing-brand" href="#top" aria-label="Bag1V-Bucks">
          ...
          <div className="landing-nav-links">
            ...
            <a href="#faq">FAQ</a>
            <a href="#live">Выдачи</a>
          </div>
          <div className="landing-nav-actions">
            <span className="landing-lang">RU</span>
            <Link href="/admin/login" className="landing-login">Войти</Link>
          </div>
        </nav>
          ...
          <div>
            ...
            <div className="landing-system-status landing-reveal" data-delay="1">
              ...
              <div>
                <span>В очереди</span>
                <div className="status-row">
                  <span className={`status-dot ${queueSize > 0 ? 'status-yellow' : 'status-green'}`} />
                  <b>{queueSize} чел.</b>
                </div>
              </div>
            </div>
        ...
        <section className="landing-section landing-reveal" id="packages">
          ...
          <div className="landing-buy-panel landing-reveal">
            ...
            <button className="landing-btn landing-btn-primary" onClick={handleBuy} type="button">
              Перейти к оплате
            </button>
          </div>
        </section>
        ...
        <section className="landing-section landing-reveal" id="live">
          ...
          <div className="landing-live-board landing-reveal">
            {liveOrders.map(([id, amount, time, status]) => (
              <div className="landing-live-row" key={id}><b>{id}</b><span>{amount}</span><span>{time}</span><span className="landing-live-status">{status}</span></div>
            ))}
          </div>
        </section>
        ...
        <footer className="landing-footer landing-reveal">
          <a className="landing-brand" href="#top"><span className="landing-brand-icon"><VbucksIcon size={18} /></span>Bag1V-Bucks</a>
          <div className="landing-footer-links"><Link href="/oferta">Оферта</Link><Link href="/privacy">Политика</Link><Link href="/cookies">Cookie policy</Link><a href="https://t.me/BAG1BAG1" target="_blank" rel="noreferrer">Telegram</a></div>
          <div>© 2026 Bag1V-Bucks · {DOMAIN}</div>
        </footer>
      </div>
